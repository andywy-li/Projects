  ...
  static int size_impl(const Node *node) {
    ...
    if(node->left){
      if(node->right){
        ...
        }else{return 1+size_impl(node->left);}
    }else{
      if(node->right){return 1+size_impl(node->right);
      }else{return 1;}
    }
  }
...
//--------------------------12/4------------------------------
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    ...
    if (right_min != nullptr && !less(node->datum, right_min->datum)) {
        return false;
    }
    Node* left_max = max_element_impl(node->left);
    if (left_max != nullptr && !less(left_max->datum, node->datum)) {
        return false;
    }
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
        ...
        if(less(val,node->datum)){
          if(node->left){
            Node* node_left=min_greater_than_impl(node->left,val,less);
            if(!node_left){return node;}
            if(less(node_left->datum,node->datum)){
              return node_left;
            }else{return node;}
          }else{
            return node;
